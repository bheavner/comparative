function [results]=testYeastModel_glu_min(model,varargin)
    ...
    if medium == 1
        ...
        % iron for iMMbs, which requires it for heme biosynthesis)
        %
        % note: this isn't a realistic medium (no H+ or H2O for example),
        % but models all predict biomass formation with just these
        % components, so it's useful for comparative evaluation.
	...
	if medium == 2
        ...
        % synthetic complete reference: Kennedy CJ, Boyle PM, Waks Z,
        % Silver PA. "Systems-Level Engineering of Nonfermentative
        % Metabolism in Yeast" Genetics (2009) 183:385-397. DOI:
        % 10.1534/genetics.109.105254

        if output
            fprintf('\nMaking synthetic complete medium...\n\n');
        end
    ...
    % combine inviable with auxotrophy list if using a minimal medium or
    % the model default medium
    if medium == 2
    ...
    %% testing auxotrophy
    % For simulations using minimal medium or model default medium, compare
    % predictions with predicitions when all exchange fluxes are
    % unconstrained (a simulated complex medium) The reference lists of
    % genes are at the end of this function.
