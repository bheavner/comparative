function [results]=testYeastModel_glu_min(model,varargin)
    ...
    %             1 = use minimal medium (glucose limiting)
    %             2 = use synthetic complete medium (ref
    ...
    %
    %   dead ends (optional)
    %             0 = do not find dead end metabolites (default)
    %             1 = find dead end metabolites (takes longer)
    %
    %   output (optional)
    %             0 = silent
    ...
    % TODO: 
    %   results structure, fix blocked reactions code
    ...
    % 5 optional inputs (6 total) at most
    ...
    [medium biomass blocked dead output] = optargs{:};
    ...
    %% create model with minimal medium 
    % ammonia/um, glucose, oxygen, phosphate, sulphate
    %
    % note: this isn't a realistic medium (no H+ or H2O for example), but
    % iFF, iND, iMM, iAZ, and Y5/6 all predict biomass formation with just
    % these components, so it's useful for testing models.
    %
    % iMMbz also needs iron, since it includes heme in the default biomass
    % definition
    %
    % iIN doesn't
    % predict growth on this medium using either the as-distributed biomass
    % definition or the iFF708 biomass either (the iFF708 biomass is
    % blocked by 1-phosphatidyl-1D-myo-inositols, phosphatidate,
    % phosphatidylcholines, phosphatidylethanolamines, phosphatidylserine,
    % and triglycerides).
    ...
    if medium
        ...
        switch lower(model.description)
            ...
            case {'imm904_nadcorrected'}
                % default biomass def also requires iron
    ...
    if blocked
        ...
        blockedReactions = findBlockedReaction_ks(model_maximal);
    ...
    % combine inviable with auxotrophy list if using a minimal medium
    ...
    %% testing auxotrophy
    % this compares model prediction of mutant growth in minimal media and
    % maximal media (all exchange fluxes allowed) to mutant growth
    % phenotype observations. The reference lists of genes are at the end
    % of this function.
    ...
    aux_inviable = model.genes(grRatio < ko_tol);
    aux_inviable = intersect(aux_inviable,auxotrophs);
    aux_inviable = setdiff(aux_inviable,aux_fail);
    ...
    if output
        ...
        fprintf('\t%g\t%s\n\t%g\t[%.2f %%]\t%s\n\t%g\t[%.2f %%]\t%s\n\t%g\t[%.2f %%]\t%s\n\n',...
            ...
            length(aux_inviable),(100*length(aux_inviable)/n_aux),...
...
function genes = inviableORFs
    % list of 1120 unique essential ORFs taken from the Yeast Deletion
    % Project (14 aug 11)
...
function genes = auxotrophicORFs
    % list of 452 auxotrophic ORFs (669 annotated auxotroph minus 52 on YKO
    % essential list minus 87 which are re either temp-sensitive ino
    % auxotrophs or not auxotrophs in the nature study plus modifications
    % as annotated below):
